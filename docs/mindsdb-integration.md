...
```sql
-- "supabase_voice_analysis" คือชื่อที่เราตั้งเอง ใช้อ้างอิงใน query ทีหลัง
...
FROM supabase_voice_analysis.audio_files af
LEFT JOIN supabase_voice_analysis.analysis_results ar ON af.id = ar.audio_file_id
...
CREATE TABLE IF NOT EXISTS voice_analysis.kb_transcriptions (
...
CREATE INDEX IF NOT EXISTS kb_transcriptions_emb_idx
  ON voice_analysis.kb_transcriptions
...
FROM supabase_voice_analysis.analysis_results ar
...
USING
  ...
  data = {
    "tables": [
      "supabase_voice_analysis.audio_files",
      "supabase_voice_analysis.analysis_results"
    ]
  },
...
DROP TABLE IF EXISTS voice_analysis.kb_transcriptions;
```
...
- [ ] สร้าง `voice_analysis.kb_transcriptions` table + index + GRANT ใน **Supabase**
